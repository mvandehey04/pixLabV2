...
public void grayscale(){
  ...
  public void mirrorArms()
  {
    ...
    {
      // loop from 13 to just before the mirror point
      for (int col = 300; col > mirrorPoint; col--)
      {
        leftPixel = pixels[row][col];      
        rightPixel = pixels[row + 62][mirrorPoint - (col - 1) + mirrorPoint];
        rightPixel.setColor(leftPixel.getColor());
      }
    }
    for (int row = 158; row < 199; row++)
    {
      // loop from 13 to just before the mirror point
      for (int col = 102; col < mirrorPoint; col++)
      {
        leftPixel = pixels[row][col];      
        rightPixel = pixels[row + 62][mirrorPoint - (col - 14) + mirrorPoint];
        rightPixel.setColor(leftPixel.getColor());
      }
    }
  }
